...
1. **Agent → ClawCoach API**: Agent logs workouts, requests coaching data, triggers $FIT rewards
...
// Trigger $FIT reward — also signed
...
export async function POST(request: Request) {
  ...
  // ... create workout record, trigger $FIT reward, etc.
}
```
...
| `ttlSeconds` | `number` | `60` | 60s for normal ops. 30s for $FIT reward triggers. |
